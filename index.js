...
exports.loadPlugin = async function(gridController, persistedData) {
    ...
    let actionHtml = fs.readFileSync(path.resolve(__dirname, 'sample_action.html'), {encoding: "utf-8"})
        controller.sendMessageToRuntime({
            id: "add-action",
            info: {
                actionId: -1,
                short: "xpluginactionstatic",
                name: "MouseMove",
                rendering: "standard",
                category: "plugin",
                desc: `Static Plugin Action`,
                blockTitle: "Static plugin action",
                defaultLua: "gmms(,)",
                color: "#9C92A4",
                icon: `
...
exports.unloadPlugin = async function() {
    ...
    messagePorts.clear()
    //Remove every button added by the plugin when it gets undloaded
    while(buttonId > 0){
        removeButton()
    }
    controller.sendMessageToRuntime({
...
//Communication with the renderer side preference.html file
...
async function onMessage(port, data) {
    if (data.type === 'add-button'){
        addButton(data.html)
    }
    if (data.type === 'remove-button'){
        removeButton()
    }
}

//Dynamically add a new action button when the user presses the button in the preference page
    ...
    try {
        ...
        controller.sendMessageToRuntime({
            ...
            info: {
                ...
                rendering: "standard",
                category: "plugin",
                desc: `Plugin ${buttonId}`,
                blockTitle: "Plugin action",
                defaultLua: "gmms(,)",
                color: "#9C92A4",
                icon: `
                <svg width="100%" height="100%" viewBox="-96 0 512 512"  xmlns="http://www.w3.org/2000/svg">
        ...
        controller.sendMessageToRuntime({id: "debug-error", stack: e.stack, message: e.message})
    }
}
